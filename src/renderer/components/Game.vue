...
<div class="game-screen">
  <div class="code-area">
    ...
    <div id="mainCode" v-bind:class="{ hidden:(codeMode != 'main')}">
        ...
        <div class="action-buttons">
            <button v-on:click="runCode" class="btn btn-dark float-right btn-run-code" 
                    :class="{'btn-retry':codeIsRunning}">
                {{codeIsRunning ? "Retry" : "Run Code"}}
            </button>
            <a @click="navigateToLevelSelect()" class="btn btn-dark" href="#">Level Select</a>
            <button v-on:click="runStory" class="btn btn-dark" :disabled="codeIsRunning">
                Story
            </button>
        </div>
    </div>
  ...
  <b-modal id="errorModal" ref="errorModal" 
    @ok="retryLevel"
    ok-title="Retry"
    ok-only
      title="Syntax Error...">
...
export default {
  ...
  mounted() {
    ...
    { name: 'run()', description: 'run([number]) - number represents the speed for Jaxi to run.' },
    { name: 'jump()', description: 'jump([number]) - number represents the strength of Jaxi\'s jump.' },
    { name: 'climb()', description: 'climb' },
    { name: 'throwSnowball()', description: 'throwSnowball(number) - number represents the angle for Jaxi to throw the snowball.' },
    { name: 'dance()', description: 'useful for waiting for flowers to grow back.' },
    { name: 'pickUp()', description: 'pickUp() - picks up an object, and returns its value.' },
    { name: 'inspect()', description: 'inspect - returns the value of a nearby object.' },
    { name: 'putDown()', description: 'putDown([string or array]) - puts down the specified object(s).' },
    { name: 'isNear()', description: 'isNear(string) - returns true if Jaxi is near the specified object.' },
    { name: 'isTouching()', description: 'isTouching(string) - returns true if Jaxi is touching a specified object.' },
    { name: 'say()', description: 'say(string) - causes Jaxi to speak.' },
    ];
    ...
    //give focus to the editor if anything but enter is hit.
    window.addEventListener("keydown", vue.keydownHandler, true);
  },
  destroyed() {
      window.removeEventListener("keydown", vue.keydownHandler, true);
  },
  methods: {
    keydownHandler: function(event)
    {
        var editor = (vue.codeMode == 'main') ? ace.edit("editor") : ace.edit("functionsEditor");
...
function onUpdateEvent()
{
    ...
    runCode: function()
    {
        ...
        {
            //alert(err.message)
            vue.errorMessage = err.message;
            vue.$refs.errorModal.show()
            return
        }
       ...
       function formatUnexpectedTokenString(message, codePanel)
       {
           //Unexpected token (3:5)
            ...
            vue.errorMessage = `It looks like there is a misplaced or missing character somewhere in your code.\n\nIt might be in your ${codePanel} code panel, on line ${lineNum}, character ${charNum}.`
            vue.$refs.errorModal.show()
       }
    ...
    dismissLevelAnimation: function()
    {
        if(vue.animationStep < vue.animationArray.length)
        {
            vue.animationStep = vue.animationArray.length - 1;
            vue.nextAnimationStep();
            if(vue.currentBubble) vue.currentBubble.destroy();
        }
    },
